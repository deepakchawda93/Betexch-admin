...
app.post("/register", upload.none(), async (req, res) => {
  try {
    ...
    if (!(username, first_name, last_name, password, contact_no)) {
      res
        .status(400)
        .json({ status: 400, message: "All input required", success: false });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total matches completed : " + matches.length,
          success: true,
          data: pagedata,
        });
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      await redisclient.set("Active_match_id_" + gameId, gameId);
      await redisclient.set("Active_BM_MID_" + marketId, marketId);
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total matches in-play : " + matches.length,
          success: true,
          matches,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total upcoming matches : " + matches.length,
          success: true,
          data: pagedata,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (stokists.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total stokists : " + stokists.length,
          success: true,
          data: pagedata,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (seniorStokists.length < 1) {
        res.status(403).json({
          status: 403,
          message: "No senior stokist data found",
          success: false,
          data: null,
        });
      } else {
        ...
        res.status(200).json({
          status: 200,
          message: "Total senior stokists : " + seniorStokists.length,
          success: true,
          data: pagedata,
        });
      }
...
app.post("/new_match", sessionCheck.isOwner, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ status: 201, message: "Match created", data: newmatch });
  } catch (err) {
    console.log(err);
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total matches inserted manually : " + matches.length,
          success: true,
          data: pagedata,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total matches inserted by API : " + matches.length,
          success: true,
          data: pagedata,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (matches.length < 1) {
        ...
        res.status(200).json({
          status: 200,
          message: "Total matches abandoned : " + matches.length,
          success: true,
          data: pagedata,
        });
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(match);
      await redisclient.del("Active_match_id_" + game_id);
      res.status(201).json({
        status: 201,
  ...
  async (req, res) => {
    try {
      ...
      if (!(market_id && team1 && team2)) {
        res
          .status(400)
          .json({ status: 400, message: "Insufficient input", success: false });
      }
      await Match.updateOne(
        { market_id },
      ...
      await Match.updateOne(
        { market_id },
        {
          $set: {
            ...
            session_api_hit: session_api_hit,
            fancy_running_type: fancy_running_type,
            score_hit_method: score_hit_method,
            client_api_hit: client_api_hit,
            date_time: date_time,
            name: name,
            tv_type: tv_type,
            socket: socket,
            bet_delay_time: bet_delay_time,
            score_id,
            funting_status,
            fav_team,
            maximum_match_amount,
            minimum_match_amount,
            maximum_session_amount: maximum_session_amount,
            minimum_session_amount: minimum_session_amount,
            team1: team1,
            team2: team2,
          },
        }
  ...
  async (req, res) => {
    try {
      ...
      if (!match) {
        ...
        res.status(400).json({
          status: 400,
          messgae: "No match corresponds to the given market_id",
          success: false,
          data: null,
        });
      } else {
  ...
  async (req, res) => {
    try {
      ...
      if (!(id && decision_run)) {
        ...
        const data = await Session.updateOne(
          { id },
          { $set: { status: "declared", decision_run: decision_run } }
        );
        console.log(data);
        res.status(201).json({
          status: 201,
          message: "Session declared",
          success: true,
          data,
        });
      }
...
app.get("/sessions", sessionCheck.isAuth, async (req, res) => {
...
app.get("/logout", sessionCheck.isOwner, upload.none(), async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: "200", message: "Owner logged-out", success: true });
});

//add sessions in inplay
app.post(
  "/add-sessions-inplay",
  sessionCheck.isOwner,
  upload.none(),
  async (req, res) => {
    ...
    try {
      let session = await Session.updateOne(
        { sessionId: sessionId, matchId: matchId },
        { $set: { status: "active" } }
      );
      await redisclient.set("Active_session_" + sessionId + matchId);
      res.status(201).json({
        status: 201,
        message: "InPlay entry success",
        success: true,
...
//remove matches in inplay
app.post(
  "/remove-sessions-inplay",
  sessionCheck.isOwner,
  upload.none(),
  async (req, res) => {
    console.log("------------>", req.body);
...
app.post("/createSession", upload.none(), async (req, res) => {
  let gameId = req.body.gameId;
  let activeMatches = await redisclient.keys("Active_match_id_*");

  try {
    cron.schedule("*/1 * * * * *", async function () {
      console.log("Executed inside cron...");
      activeMatches.forEach(async (element) => {
        console.log("element=========>", element);
        let matchId = await redisclient.get(element);

        let url = `http://172.105.63.10:3000/getdata/${matchId}`;
        axios.get(url).then(async (res) => {
          let data = res.data;
          await redisclient.set(
            "active_session_Data_" + matchId,
            JSON.stringify(data)
          );
        });
      });
    ...
    let activeSessionData = JSON.parse(
      await redisclient.get("active_session_Data_" + gameId)
    );
    let activeSessions = [];

    activeSessionData["f"].forEach(async (sessions) => {
      activeSessions.push(sessions);
    });

    await redisclient.set(
      "active_match_session_" + gameId,
...
//api for getting match bhav
app.post("/activeMatchDetails", upload.none(), async (req, res) => {
  let gameId = req.body.gameId;
  let activeMatches = await redisclient.keys("Active_match_id_*");
  console.log("Active_match_id_------------->", activeMatches);
  cron.schedule("*/1 * * * * *", async function () {
    console.log("Executed inside cron...");
    activeMatches.forEach(async (element) => {
      let matchId = await redisclient.get(element);
  ...
  let activeMatchData = JSON.parse(
    await redisclient.get("active_Match_Data_" + gameId)
  );
  console.log("activeMatchData=============>", activeMatchData);
  let matchResults = [];

  activeMatchData["bm"].forEach(async (_bm1) => {
    _bm1["bm1"].map(async (_data) => {
      matchResults.push(_data);
    });
  });

  await redisclient.set(
    "active_match_bm_" + gameId,
...
app.get("/getSession", async (req, res) => {
  let redisData = await redisclient.get("matchData");
  ...
  redisclient.set("matchOdds", matchOdds);
